...
public final class ServerModel {
    ...
    public int getUserId(String nickname) {
        if (map != null && map.containsValue(nickname)) {
            for (Integer key : map.keySet()) {
                if (map.get(key).equals(nickname)) {
                    return key;
                }
            }
        }
    ...
    public String getNickname(int userId) {
        return map.get(Integer.valueOf(userId));
    }
    ...
    public Collection<String> getChannels() {
        ...
        for (Channel channel : channels) {
            String chan = channel.getChannelName();
    ...
    public Collection<String> getUsersInChannel(String channelName) {
        ...
        for (Channel channel : channels) {
            if (channelName.equals(channel.getChannelName())) {
                output.addAll(channel.getUsers());
                return output;
            }
        }
        return output;
    ...
    public String getOwner(String channelName) {
        for (Channel channel : channels) {
            if (channelName.equals(channel.getChannelName())) {
                return channel.getOwnerOfChannel();
            }
        }
        return null;
    }
    ...
    public Broadcast deregisterUser(int userId) {
        ...
        Set<String> notifiedUsers = new TreeSet<>();
        for (Channel channel : channels) {
            if (channel.getUsers().contains(name)) {
                channel.removeUser(name);
                notifiedUsers.addAll(channel.getUsers());
            }

            if (name.equals(channel.getOwnerOfChannel())) {
                channels.remove(channel);
            }
        }
        map.remove(userId);
        return Broadcast.disconnected(name, notifiedUsers);
    }
    ...
    public Broadcast changeNickname(NicknameCommand nickCommand) {
        ...
        } else if (map.containsValue(newNick)) {
            return Broadcast.error(nickCommand, ServerResponse.NAME_ALREADY_IN_USE);
        }
        Integer senderID = nickCommand.getSenderId();
        String currName = map.get(senderID);
        map.put(senderID, newNick);

        Set<String> recipients = new TreeSet<String>();

        for (Channel channel : channels) {
            if (channel.getUsers().contains(currName)) {
                recipients.addAll(channel.getUsers());
                channel.removeUser(currName);
                channel.addUser(newNick);
            }
            if (channel.getOwnerOfChannel().equals(currName)) {
                channel.setOwner(newNick);
            }
        }
    ...
    public Broadcast createChannel(CreateCommand createCommand) {
        ...
        boolean channelPrivacy = createCommand.isInviteOnly();
        ...
        Channel newChan = new Channel(chanName, ownerName, channelPrivacy);
        channels.add(newChan);

        Set<String> notifiedUser = new TreeSet<>();
        notifiedUser.add(ownerName);

        return Broadcast.okay(createCommand, notifiedUser);
    }
    ...
    public Broadcast joinChannel(JoinCommand joinCommand) {
        ...
        for (Channel channel : channels) {
            if (channel.getChannelName().equals(channelName)) {
                ...
                channel.addUser(senderName);
                Set<String> recipients = new TreeSet<>();
                recipients.addAll(channel.getUsers());
                return Broadcast.names(joinCommand,
                        recipients, channel.getOwnerOfChannel());
            }
        }
    ...
    public Broadcast sendMessage(MessageCommand messageCommand) {
        ...
        for (Channel channel : channels) {
            if (channel.getChannelName().equals(channelName)) {
                if (channel.getUsers().contains(map.get(senderId))) {
                    Set<String> recipients = new TreeSet<>();
                    recipients.addAll(channel.getUsers());
    ...
    public Broadcast leaveChannel(LeaveCommand leaveCommand) {
        ...
        Integer senderId = leaveCommand.getSenderId();
        ...
        for (Channel channel : channels) {
            if (channel.getChannelName().equals(channelName)) {
                if (channel.getUsers().contains(map.get(senderId))) {
                    Set<String> recipients = new TreeSet<>();
                    recipients.addAll(channel.getUsers());
                    channel.removeUser(map.get(senderId));
    ...
    public Broadcast inviteUser(InviteCommand inviteCommand) {
        ...
        String receiver = inviteCommand.getUserToInvite();

        if (!map.containsValue(receiver)) {
            return Broadcast.error(inviteCommand, ServerResponse.NO_SUCH_USER);
        }

        for (Channel channel : channels) {
            boolean sameChannelName = channel.getChannelName().equals(channelName);
            boolean sameSenderName = channel.getOwnerOfChannel().equals(map.get(senderId));
            boolean channelPrivacy = channel.getIsPrivate();

            if (sameChannelName && sameSenderName && channelPrivacy) {
                channel.addUser(receiver);
                Set<String> recipients = new TreeSet<>();
                recipients.addAll(channel.getUsers());
                return Broadcast.names(inviteCommand, recipients, channel.getOwnerOfChannel());
            } else if (!sameSenderName) {
                return Broadcast.error(inviteCommand, ServerResponse.USER_NOT_OWNER);
            } else if (!channelPrivacy) {
